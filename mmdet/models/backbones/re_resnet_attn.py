...
class ReResNetAttn(BaseBackbone):
    ...
    def __init__(self,
                 ...
                 orientation=8,
                 fixparams=False):
        super(ReResNetAttn, self).__init__()
        ...
        ################
        self.tokens = nn.ModuleList([nn.Embedding(1,256),nn.Embedding(1,512),nn.Embedding(1,1024),nn.Embedding(1,2048)])
    ...
    def init_weights(self, pretrained=None):
        super(ReResNetAttn, self).init_weights(pretrained)
    ...
    def forward(self, x):
        ...
        for i, layer_name in enumerate(self.res_layers):
            ...
            x = res_layer(x)
            token_i = self.tokens[i].weight[None,:,:].repeat(x.shape[0],1,1)
            attn_map = ((torch.bmm(token_i,x.tensor.view(x.shape[0],x.shape[1],-1)))/math.sqrt(256)).softmax(-1)# 1*(256*256)
            x_refine = (attn_map*(x.tensor.view(x.shape[0],x.shape[1],-1))).reshape(x.shape[0],x.shape[1],x.shape[2],x.shape[3])
            x.tensor = x.tensor+x_refine
    ...
    def train(self, mode=True):
        super(ReResNetAttn, self).train(mode)
        self._freeze_stages()
